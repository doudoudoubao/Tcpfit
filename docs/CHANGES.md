...
第一轮改完之后又通读了一遍, 这几条是那轮遗留的（有两条是我自己引入的）:

| # | 问题 | 影响 | 改法 |
|---|---|---|---|
| 1 | `take_snapshot` 里仍是 `die`, 而 `harden_swap` 会调它 | "有 tcpfit 配置但无快照"的机器上, 菜单 4 / 向导结尾加 swap 会**整个脚本退出** —— 正是本分支要消灭的那一类, 上一轮漏了 | `take_snapshot` 改为 `return 1`; 命令级调用方(`tune`/`shape`)自己 `die`, `harden_swap` 只跳过这一步 |
| 2 | `die(){ ... "$*" ... }` 把第二个参数(退出码)也打进了消息 | `die "已中止, 未做任何改动" 1` 在屏幕上显示成 `已中止, 未做任何改动 1`. 上游 bug, 10 处调用受影响 | 改用 `$1` |
| 3 | 向导里 `sweep_preflight` 在 `confirm "开始调优？"` **之前** | 可能已经建好 2 GB swapfile、写了 fstab、存了快照, 用户再选 n, 屏幕却说"已取消, 未做任何改动" | preflight 加 `defer` 模式: 确认之前只问不做, 答案记在 `PREFLIGHT_SWAP_GB`, 确认之后由 `preflight_apply()` 执行 |
| 4 | `guard --off` 用 `pkill -x iperf3` | 分不出客户端和服务端, 会把用户常驻的 `iperf3 -s` 一起干掉 | 只 kill 命令行里含 ` -c ` 的进程 |
| 5 | `guard --off` 不看有没有 sweep 正在跑 | 另一个终端正在测速时执行它, 会把对方的 qdisc 抽走, 那一轮结果作废且无人知晓 | 心跳在 `GUARD_TTL` 内就先警告并要求确认(默认 n) |
| 6 | `systemd-run --unit=tcpfit-guard` 遇到同名残留单元会失败, 而错误被吞掉 | systemd 那层硬兜底**静默失效**, 只剩看门狗进程一层 | 武装前 `stop timer + service` 并 `reset-failed` |
| 7 | 单核机上 `VPAR` 被压到 1, 验证表把同一组数打两行 | 只是难看 | `VPAR > 1` 时才打第二行 |

第 1、3 条是这一轮里真正重要的 —— 它们是同一个毛病的复发:
**"收尾步骤里的失败不该掀翻整张桌子", 以及"用户点头之前不要动系统"**.
...
| 开机 qdisc 脚本 | 生成后 `bash -n` + shellcheck + 对着打桩 tc 跑两种速率 | 班道结构与过滤器条数(2 + 端口数×4)都对 |
| `take_snapshot` 失败路径 | 造一台"有配置无快照"的机器, 调 `harden_swap` | 返回调用方, 后续逻辑照常跑 |
| `guard --off` | 起一个不含 `-c` 的 iperf3 进程 | 未被误杀; 心跳在有效期内时要求确认 |
