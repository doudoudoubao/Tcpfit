#!/usr/bin/env bash
# tcpfit 一键部署（doudoudoubao/Tcpfit 分支, 上游 Kylin010/tcpfit, MIT）
#
# 两种用法：
#
#   1) 在目标 VPS 上直接跑（只装 agent, 单机调优）
#      curl -fsSL https://raw.githubusercontent.com/doudoudoubao/Tcpfit/main/install.sh | bash
#      然后: tcpfit detect
#
#   2) 在控制端跑（装完整项目, 管理多台机器 —— 未上线, 尚未在真实环境验证）
#      curl -fsSL https://raw.githubusercontent.com/doudoudoubao/Tcpfit/main/install.sh | bash -s -- --full
#      然后: cd /opt/tcpfit && python3 orchestrator/fleet.py detect

set -euo pipefail

REPO="doudoudoubao/Tcpfit"
...
if [ "$MODE" = agent ]; then
  ...
  echo "    tcpfit guard --off                  # 被测试限速卡住时的急救"
